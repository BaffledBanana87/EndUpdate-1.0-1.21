package net.baffledbanana87.repurposedend.entity.witherskull;
...
public class SkullPlacedEvent extends SkullBlock{

    @Nullable
    private static BlockPattern witherBossPattern;

    private static final Predicate<BlockState> IS_WITHER_HEAD = state -> state != null
            && (state.isOf(Blocks.WITHER_SKELETON_SKULL));



    public SkullPlacedEvent(AbstractBlock.Settings settings) {
    ...
    public static void onPlaced(World world, BlockPos pos) {
        if (world.getBlockEntity(pos) instanceof SkullBlockEntity skullBlockEntity) {
            ...
            SkullPlacedEvent.onPlaced(world, pos, skullBlockEntity);
        }
    }
    ...
    public static void onPlaced(World world, BlockPos pos, SkullBlockEntity blockEntity) {
        if (!world.isClient) {
            ...
            if (bl && pos.getY() >= world.getBottomY() && world.getDifficulty() != Difficulty.PEACEFUL) {
                ...
                if (result != null) {
                    CryingWitherEntity witherEntity = ModEntity.CRYING_WITHER.create(world);
        ...
        if (witherBossPattern == null) {
            witherBossPattern = BlockPatternBuilder.start()
                    ...
                    .where('#', CachedBlockPosition.matchesBlockState(
                            BlockStatePredicate.forBlock(Blocks.CRYING_OBSIDIAN)))
                    .where(
                            '^',
                            CachedBlockPosition.matchesBlockState(IS_WITHER_HEAD)
                    )
